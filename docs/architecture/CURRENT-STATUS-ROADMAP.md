...
| Test baseline | 240 files / 643 tests passing | Latest full `pnpm test` run after v0.59 production runtime persistence bootstrap |
...
1. Auth and account persistence
   ...
   - Next replace JSON/local session/account stores with authenticated account/workspace database storage and production cookie/secret rotation.
