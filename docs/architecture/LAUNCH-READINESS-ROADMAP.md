...
- Deployed certified smoke policy: the step is blocking on main pushes, because the temporary origin auto-deploys from main through the Vercel Git integration; verified refreshed on 2026-07-16 (`/en/tools/percentage-calculator` 200, main deployed smoke green). Since 2026-07-19 the CI follows a hard minute budget after the Actions limit was exhausted: push/PR (code paths only, docs skipped) run a ~6-minute fast static gate (typecheck, full unit suite, i18n/button audits); the 31-minute full launch gate runs nightly on schedule and on workflow_dispatch, capped near ~1.5k minutes/month worst case.
