...
- Deployed certified smoke policy: the step is blocking on main pushes, because the temporary origin auto-deploys from main through the Vercel Git integration; verified refreshed on 2026-07-16 (`/en/tools/percentage-calculator` 200, main deployed smoke green). Since 2026-07-19 the CI is split for Actions-minute budget: PRs run a fast static gate (typecheck, full unit suite, i18n/button audits, no build or browser), while the full launch gate with browser gates and deployed smoke runs on main pushes only.
