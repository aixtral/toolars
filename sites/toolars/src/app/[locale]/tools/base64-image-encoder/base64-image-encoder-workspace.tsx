  ...
  return (
    <AiLabWorkbenchShell artifactState={result.isValid ? t("artifact.ready") : t("artifact.blocked")} providerRoute={t("providerRoute")} runMode={t("runMode")} toolSlug="base64-image-encoder">
      ...
      <main className="workspace-stack">
        ...
        <section className="workspace-panel"><h2>{t("resultTitle")}</h2><p className="tool-description">{result.mimeType} | {result.byteSize.toLocaleString(locale)} {rootT("tools.file-size-converter.workspace.bytesLabel")}</p><pre aria-label={t("outputLabel")} className="textarea prompt-textarea">{result.dataUrl}</pre></section>
      </main>
