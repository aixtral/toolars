...
import { calculatePhq9Depression, defaultPhq9Answers, type Phq9Severity } from "./phq9-depression";
...
describe("calculatePhq9Depression", () => {
  it("scores the default PHQ-9 answers and maps them to the source mild band", () => {
    ...
    expect(result.severity).toBe<Phq9Severity>("mild");
  ...
  it("flags item 9 self-harm risk even when the total score is low", () => {
    ...
    expect(result.severity).toBe<Phq9Severity>("minimal");
